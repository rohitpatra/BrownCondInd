...
```
 ...
 Here  ``cond.dist.obj'' is the the list conataining estimators of $F_{X|Z}$, $F_{Y|Z}$, and $F_Z$ evaluated at the data points (denoted by F.x_z, F.y_z, and F.z_hat) and the bandwidth used (denoted by Fbw.x_z, Fbw.y_z, and Fbw.z_z) to evaluate the conditional distribution fucntions. Note that we use the functions available in the "np'' package ( see @np) to compute the optimal bandwiths as well as the estimates of the conditional distribution functions.  
...
The required R file ``Npres_Fucntions.R''  can be downloaded [here](http://stat.columbia.edu/~rohit/Code/Npres_Fucntions.R). Note the fucntions require the following R-packages: boot, data.table, energy, np, and stats.
